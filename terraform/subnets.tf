resource "aws_subnet" "MidProject-Subnet-Priv" {
  ...
  cidr_block        = "${cidrsubnet(aws_vpc.OpsSchool-MidProject-VPC.cidr_block, 8, count.index)}" #(understand)
...
resource "aws_subnet" "MidProject-Subnet-Pub" {
  count                   = "2"
  availability_zone       = "${element(var.azs,count.index)}"
  cidr_block              = "${cidrsubnet(aws_vpc.OpsSchool-MidProject-VPC.cidr_block,8,count.index + 2)}" #(understand)
